...
from kivymd.uix.button import MDRoundFlatIconButton, MDFlatButton, MDIconButton, MDRoundFlatButton, MDRectangleFlatIconButton
...
from kivymd.uix.screen import MDScreen  #VERSION OF LIBRARIES: KivyMD = 1.1.1 , Kivy = 2.3.1 , SymPy = 1.10.1
from kivy.lang.builder import Builder            #update tag: Panda, 2026
...
mykeyboard_keys={'b1': ('∫ dx','#EEEEEE'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
            'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('(','#DEDEDE'), 'b8':(')','#DEDEDE'),'b9':('7','F6F6F6'),
...
old_mykeyboard_keys=mykeyboard_keys.copy()  #after changes, turn back main keyboard
...
class MenuScreen(Screen,TouchBehavior):  #we take touchBehaviour from kivymd
    ...
    from stack_over_function import oto_width_main, oto_height_main  #--> call the height and width of math symbols on label.       
    ...
    def checkfocus(self):
        ...
        if check_focus==True and self.ids.txt1.keyboard_mode=='managed': #call our created keyboard
            ...
            mykeyboard=MDGridLayout(cols=6, rows=5, height='320dp',md_bg_color='#EEEEEE', spacing='0.50dp', size_hint_y=None)
            ...
            for n,m in mykeyboard_keys.items():
                but=Button(text=str(m[0]), color='black', font_size='22sp' ,markup=True, font_name='dejavusansmono.ttf', background_normal=m[1], background_color=m[1],size_hint_y=None, height='64dp')
                mykeyboard.add_widget(but)                          
                but.bind(on_press= lambda _,z=str(m[0]):self.mykeyborad_click(z))
                if n=='b4':
                    self.ids['key_cursor_left']=but
                if n=='b6':
                    self.ids['key_delete']=but #for check state of delete button.
                
                if n=='b5':
                    self.ids['key_cursor_right']=but
    ...
    def on_long_touch(self, *args): #delete textbox with long-touch
        try:
            ...
            if self.ids.key_delete.state=='down' and l != 0 :
                ...
                Clock.schedule_once(self.on_long_touch, 0.025)
                self.ids.txt1.cursor = self.ids.txt1.get_cursor_from_index(l-1)
                i = self.ids.txt1.cursor_index()
                self.ids.txt1.text = (self.ids.txt1.text[:i] + self.ids.txt1.text[i+1:])
                self.ids.txt1.cursor = self.ids.txt1.get_cursor_from_index(i)
                #self.ids.txt1.text=''
            ...
            elif self.ids.key_cursor_left.state=='down' and l!=0: #left , right cursor
                self.ids.txt1.focus=True
                Clock.schedule_once(self.on_long_touch, 0.06)
                self.ids.txt1.cursor=self.ids.txt1.get_cursor_from_index(l-1)
            elif self.ids.key_cursor_right.state=='down' and l!=0: 
                self.ids.txt1.focus=True
                Clock.schedule_once(self.on_long_touch, 0.06)
                self.ids.txt1.cursor=self.ids.txt1.get_cursor_from_index(l+1)
    ...
    def mykeyborad_click(self, z): 
        try:
            ...
            'tanh','coth','[size=40]asinh[/size]','[size=40]acosh[/size]','[size=40]atanh[/size]','[size=40]acoth[/size]',
            'det','[size=24]transpose[/size]','[size=28]inverse[/size]','[size=18]eigenvalues[/size]','[size=18]eigenvectors[/size]',
            '[size=26]cofactor[/size]','[size=26]echolon[/size]','rank']
            ...
            if z=='[size=82]←[/size]':
                ...
                add_bra2=add_bra1.replace('[size=30]','')
                k=len(add_bra2)
                l=self.ids.txt1.cursor_index()
                ...
                my_matrix=z
                my_matrix1=my_matrix.replace('[size=24]transpose[/size]','.T')
                my_matrix2=my_matrix1.replace('[size=28]inverse[/size]','.inv()')
                my_matrix3=my_matrix2.replace('[size=18]eigenvalues[/size]','.eigenvals()')
                my_matrix4=my_matrix3.replace('[size=18]eigenvectors[/size]','.eigenvects()')
                now_matrix=my_matrix4.replace('[size=26]echolon[/size]','.echelon_form()')
                now_matrix1=now_matrix.replace('rank','.rank()')
                now_matrix2=now_matrix1.replace('det','.det()')
                now_matrix3=now_matrix2.replace('[size=26]cofactor[/size]','.cofactor_matrix()')
                k=len(now_matrix3)
                l=self.ids.txt1.cursor_index()
                ...
                type_hint_text.clear()
                type_hint_text.append('Sample: [color=#4F49FF]integrate[/color](cos(x), x)')
                self.ids.keyboard_hint.text='Sample: [color=#4F49FF]integrate[/color](cos(x), x)'
                mykeyboard_keys.update(old_mykeyboard_keys)
                return self.ids.box_layout.remove_widget(artifical_keyboard), self.checkfocus(), self.pretty_symbols(a=first_text_check_for_pretty,b=self.ids.txt1.text)
            elif z == '[sub]⬚[/sub]∫[sup]⬚[/sup] ⬚':
                l=self.ids.txt1.cursor_index()
                self.ids.txt1.text = self.ids.txt1.text[:l]+'integrate(f, (x, a, b)'+self.ids.txt1.text[l:]
                type_hint_text.clear()
                type_hint_text.append('Sample: [color=#4F49FF]integrate[/color](cos(y), (y, 0, pi))')
                self.ids.keyboard_hint.text='Sample: [color=#4F49FF]integrate[/color](cos(y), (y,0,pi))'
                mykeyboard_keys.update(old_mykeyboard_keys)
                return self.ids.box_layout.remove_widget(artifical_keyboard), self.checkfocus(), self.pretty_symbols(a=first_text_check_for_pretty,b=self.ids.txt1.text)
            elif z == '∫∫⬚':
                l=self.ids.txt1.cursor_index()
                self.ids.txt1.text = self.ids.txt1.text[:l]+'integrate(f, (x), (y)'+self.ids.txt1.text[l:]
                type_hint_text.clear()
                type_hint_text.append('Sample: [color=#4F49FF]integrate[/color](x + y, (x), (y))')
                self.ids.keyboard_hint.text='Sample: [color=#4F49FF]integrate[/color](x + y, (x), (y))'
                mykeyboard_keys.update(old_mykeyboard_keys)
                return self.ids.box_layout.remove_widget(artifical_keyboard), self.checkfocus(), self.pretty_symbols(a=first_text_check_for_pretty,b=self.ids.txt1.text)
            elif z=='[u] d⬚[/u] \n dx': 
                l=self.ids.txt1.cursor_index()
                ...
                type_hint_text.clear()
                type_hint_text.append('Sample: [color=#4F49FF]diff[/color](x+2)')
                self.ids.keyboard_hint.text='Sample: [color=#4F49FF]diff[/color](x+2)'
                mykeyboard_keys.update(old_mykeyboard_keys)
                return self.ids.box_layout.remove_widget(artifical_keyboard), self.checkfocus(), self.pretty_symbols(a=first_text_check_for_pretty,b=self.ids.txt1.text)
            elif z=='[u] d[/u][sup]⬚[/sup][u]⬚[/u] \n dx[sup]⬚[/sup]':
                l=self.ids.txt1.cursor_index()
                self.ids.txt1.text = self.ids.txt1.text[:l]+'diff(f, x, n'+self.ids.txt1.text[l:]
                type_hint_text.clear()
                type_hint_text.append('Sample: [color=#4F49FF]diff[/color](cos(x), x, 2)')
                self.ids.keyboard_hint.text='Sample: [color=#4F49FF]diff[/color](cos(x), x, 2)'
                mykeyboard_keys.update(old_mykeyboard_keys)
                return self.ids.box_layout.remove_widget(artifical_keyboard), self.checkfocus(), self.pretty_symbols(a=first_text_check_for_pretty,b=self.ids.txt1.text)
            elif z=='lim\n[sup] x → ⬚[/sup]':
                l=self.ids.txt1.cursor_index()
            ...
            elif z=='[size=56]∑[/size][size=29]f(⬚)[/size]':
                l=self.ids.txt1.cursor_index()
                ...
                mykeyboard_keys.update(old_mykeyboard_keys)
                return self.ids.box_layout.remove_widget(artifical_keyboard), self.checkfocus(), self.pretty_symbols(a=first_text_check_for_pretty,b=self.ids.txt1.text)
            elif z=='[size=17]ass.legendre[/size]':
                l=self.ids.txt1.cursor_index()
                ...
                mykeyboard_keys.update(old_mykeyboard_keys)
                return self.ids.box_layout.remove_widget(artifical_keyboard), self.checkfocus(), self.pretty_symbols(a=first_text_check_for_pretty,b=self.ids.txt1.text)
            elif z=='[size=17]ass.laguerre[/size]':
                l=self.ids.txt1.cursor_index()
            ...
            elif z == '[color=#173EFF]∫⬚dx[/color]':
                specialfunc_keyboard={'b1': ('[color=#5A75FF]∫ dx[/color]','#FFFFFF'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('∫⬚dx','#DEDEDE'), 'b8':('[sub]⬚[/sub]∫[sup]⬚[/sup] ⬚','#DEDEDE'),'b9':('∫∫⬚','#DEDEDE'),
            ...
            elif z == '[color=#173EFF][u] d⬚[/u] \n dx[/color]': 
                specialfunc_keyboard={'b1': ('[color=#5A75FF]∫ dx[/color]','#FFFFFF'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('[u] d⬚[/u] \n dx','#DEDEDE'), 'b8':('[u] d[/u][sup]⬚[/sup][u]⬚[/u] \n dx[sup]⬚[/sup]','#DEDEDE'),'b9':('','F6F6F6'),
                'b10':('','F6F6F6'),'b11':('','F6F6F6'),'b12':('','F6F6F6'),'b13':('','F6F6F6'), 'b14':('','F6F6F6'),
                'b15':('','F6F6F6'),'b16':('','F6F6F6'),'b17':('','F6F6F6'), 'b18':('','F6F6F6'),'b19':('','F6F6F6'),'b20':('','F6F6F6'),
                'b21':('','F6F6F6'), 'b22':('','F6F6F6'),'b23':('','F6F6F6'),'b24':('','F6F6F6'),'b25':('','F6F6F6'), 'b26':('','F6F6F6'),
                'b27':('','F6F6F6'),'b28':('','F6F6F6'),'b29':('','F6F6F6'), 'b30':('','F6F6F6')}
            ...
            elif z=='[size=27][color=#173EFF]specaial\n func.[/color][/size]':
                specialfunc_keyboard={'b1': ('[color=#5A75FF]∫ dx[/color]','#FFFFFF'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('gamma','#DEDEDE'), 'b8':('zeta','#DEDEDE'),'b9':('[size=30]jacobi[/size]','#DEDEDE'),
                'b10':('[size=26]legendre[/size]','#DEDEDE'),'b11':('[size=17]ass.legendre[/size]','#DEDEDE'),'b12':('[size=28]hermite[/size]','#DEDEDE'),'b13':('[size=25]laguerre[/size]','#DEDEDE'), 'b14':('[size=17]ass.laguerre[/size]','#DEDEDE'),
                'b15':('','F6F6F6'),'b16':('','F6F6F6'),'b17':('','F6F6F6'), 'b18':('','F6F6F6'),'b19':('','F6F6F6'),'b20':('','F6F6F6'),
                'b21':('','F6F6F6'), 'b22':('','F6F6F6'),'b23':('','F6F6F6'),'b24':('','F6F6F6'),'b25':('','F6F6F6'), 'b26':('','F6F6F6'),
                'b27':('','F6F6F6'),'b28':('','F6F6F6'),'b29':('','F6F6F6'), 'b30':('','F6F6F6')}
            ...
            elif z=='[size=20][color=#173EFF]Matrix\n operators[/color][/size]':
                matrix_keyboard={'b1': ('[color=#5A75FF]∫ dx[/color]','#FFFFFF'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('det','#DEDEDE'), 'b8':('[size=24]transpose[/size]','#DEDEDE'),'b9':('[size=28]inverse[/size]','#DEDEDE'),
                'b10':('[size=18]eigenvalues[/size]','#DEDEDE'),'b11':('[size=18]eigenvectors[/size]','#DEDEDE'),'b12':('[size=26]cofactor[/size]','#DEDEDE'),'b13':('[size=26]echolon[/size]','#DEDEDE'), 'b14':('rank','#DEDEDE'),
                'b15':('','F6F6F6'),'b16':('','F6F6F6'),'b17':('','F6F6F6'), 'b18':('','F6F6F6'),'b19':('','F6F6F6'),'b20':('','F6F6F6'),
                'b21':('','F6F6F6'), 'b22':('','F6F6F6'),'b23':('','F6F6F6'),'b24':('','F6F6F6'),'b25':('','F6F6F6'), 'b26':('','F6F6F6'),
                'b27':('','F6F6F6'),'b28':('','F6F6F6'),'b29':('','F6F6F6'), 'b30':('','F6F6F6')}
                ...
                bigger_keyboard={'b1': ('[color=#5A75FF]∫ dx[/color]','#FFFFFF'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('>','#DEDEDE'), 'b8':('<','#DEDEDE'),'b9':('≥','#DEDEDE'),
                ...
                lambda_keyboard={'b1': ('[color=#5A75FF]∫ dx[/color]','#FFFFFF'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('∇[i]f[/i]','#DEDEDE'), 'b8':('∇•[i]f[/i]','#DEDEDE'),'b9':('∇×[i]f[/i]','#DEDEDE'),
                'b10':('','F6F6F6'),'b11':('','F6F6F6'),'b12':('','F6F6F6'),'b13':('','F6F6F6'), 'b14':('','F6F6F6'),
                'b15':('','F6F6F6'),'b16':('','F6F6F6'),'b17':('','F6F6F6'), 'b18':('','F6F6F6'),'b19':('','F6F6F6'),'b20':('','F6F6F6'),
                'b21':('','F6F6F6'), 'b22':('','F6F6F6'),'b23':('','F6F6F6'),'b24':('','F6F6F6'),'b25':('','F6F6F6'), 'b26':('','F6F6F6'),
                'b27':('','F6F6F6'),'b28':('','F6F6F6'),'b29':('','F6F6F6'), 'b30':('','F6F6F6')}
                ...
                _keyboard_back += 1
                abc_keyboard={'b1': ('∫ dx','#EEEEEE'), 'b2':('[color=#5A75FF]abc[/color]','#FFFFFF'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('a','#DEDEDE'), 'b8':('b','#DEDEDE'),'b9':('c','#DEDEDE'),
                ...
                _keyboard_back += 1 
                calculus_keyboard={'b1': ('[color=#5A75FF]∫ dx[/color]','#FFFFFF'), 'b2':('abc','#EEEEEE'), 'b3':('[size=38]trigo∆[/size]','#EEEEEE'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('[color=#173EFF]∫⬚dx[/color]','#DEDEDE'), 'b8':('[color=#173EFF][u] d⬚[/u] \n dx[/color]','#DEDEDE'),'b9':('lim\n[sup] x → ⬚[/sup]','#DEDEDE'),
                'b10':('[size=68]∑[/size]⬚','#DEDEDE'),'b11':('[sup]⎡1 0 0⎤\n⎢0 1 0⎟\n⎣0 0 1⎦[/sup]','#DEDEDE'),'b12':('[color=#173EFF]∇×[i]f[/i][/color]','#DEDEDE'),'b13':('[color=#173EFF]< >[/color]','#DEDEDE'), 'b14':('∞','#DEDEDE'),
                'b15':('[size=56]∑[/size][size=29]f(⬚)[/size]','#DEDEDE'),'b16':('[size=32]exp(⬚)[/size]','#DEDEDE'),'b17':('ln⬚','#DEDEDE'), 'b18':('log[sub]⬚[/sub]⬚','#DEDEDE'),'b19':('!','#DEDEDE'),'b20':('|⬚|','#DEDEDE'),
                'b21':('[sup]⬚[/sup]√⬚','#DEDEDE'), 'b22':('⎛n⎞\n⎝k⎠','#DEDEDE'),'b23':('&bl;','#DEDEDE'),'b24':(']','#DEDEDE'),'b25':('Z[sup] *[/sup]','#DEDEDE'), 'b26':('[size=20][color=#173EFF]Matrix\n operators[/color][/size]','#DEDEDE'),
                'b27':('[size=26]polydiv[/size]','#DEDEDE'),'b28':('[size=27]fourier\n series[/size]','#DEDEDE'),'b29':('[size=27]fourier\n trans.[/size]','#DEDEDE'), 'b30':('[size=27][color=#173EFF]specaial\n func.[/color][/size]','#DEDEDE')}
            ...
            elif z=='[size=38]trigo∆[/size]' or z=='[color=#5A75FF][size=38]trigo∆[/size][/color]':
                _keyboard_back += 1
                trygonometry_keyboard={'b1': ('∫ dx','#EEEEEE'), 'b2':('abc','#EEEEEE'), 'b3':('[color=#5A75FF][size=38]trigo∆[/size][/color]','#FFFFFF'), 'b4':('[size=82]←[/size]','#EEEEEE'), 
                'b5':('[size=82]→[/size]','#EEEEEE'), 'b6':('[size=86]⌫[/size]','#EEEEEE'), 'b7':('sin','#DEDEDE'), 'b8':('cos','#DEDEDE'),'b9':('tan','#DEDEDE'),
                ...
                'b21':('sinh','#DEDEDE'), 'b22':('cosh','#DEDEDE'),'b23':('tanh','#DEDEDE'),'b24':('coth','#DEDEDE'),'b25':('[size=40]asinh[/size]','#DEDEDE'), 'b26':('[size=40]acosh[/size]','#DEDEDE'),
                'b27':('[size=40]atanh[/size]','#DEDEDE'),'b28':('[size=40]acoth[/size]','#DEDEDE'),'b29':('','F6F6F6'), 'b30':('','F6F6F6')}

                mykeyboard_keys.update(trygonometry_keyboard)
                if _keyboard_back >= 2 and z=='[color=#5A75FF][size=38]trigo∆[/size][/color]':
                    mykeyboard_keys.update(old_mykeyboard_keys)
                    _keyboard_back = 0
    ...
    def button_copy(self, name):
        try:

            if name =='in':
                Clipboard.copy(self.ids.txt1.text)

            elif name =='out':
                t0 = self.ids.txt1.text.replace("×","*")
                t1 = (sympify(t0)).doit()
                t2 = str(t1).replace('**','^')
                t3 = t2.replace('*', '×')
                try:
                    if len(sympify(self.ids.lbl1.text).atoms(Number)) != 0 and len(sympify(self.ids.lbl1.text).atoms(Symbol)) == 0 and len(sympify(self.ids.lbl1.text).atoms(Function)) ==0 :
                        te = sympify(self.ids.lbl1.text).atoms(Number)
                        t3 = str(list(te)[0])
                except(Exception, SympifyError):
                    pass
    ...
    def setting_keyboard(self, p):
        try:
            ...
            if self.ids.keyboard_icon.icon=='keyboard' and p==1:
                ...
                webbrowser.open('https://www.youtube.com/@mathpathsolver/shorts')
        except(Exception):
            self.ids.txt1.keyboard_mode='auto'
        ...
        def other_mathematics(self):
            try:
                if "gamma(" in self.ids.txt1.text and self.ids.txt1.text[-1]==")":
                    ...
                    or 'sqrt(' in main_sign2 or 'root(' in main_sign2 or 'pi' in main_sign2 or '/' in main_sign2 or [main_sign2 for main_sign2 in pretty_symb[:22]]) and ("integrate(" not in main_sign2 and "limit(" not in main_sign2 and "diff(" not in main_sign2)):
    ...
    def calc_codeline(self):
        ctnt=BoxLayout(orientation='vertical')
        btn_box=BoxLayout(orientation='horizontal', size_hint_y=0.125, spacing='16dp')
        self.codetext=TextInput(text=codeline_data[-1], hint_text='Python CodeLine\nIncluded mathematical libraries: SymPy, Matplotlib\nDo not forget to use print() to get output')   
        self.code_del=MDRectangleFlatIconButton(text='Clear', icon='trash-can-outline')   
        self.code_doc=MDRectangleFlatIconButton(text='Document', icon='file-document-outline')
        self.code_run=MDRectangleFlatIconButton(text='Run', icon='play-speed')
        
        self.code_del.bind(on_release=lambda _,x=x:self.codeline(x=1))
        self.code_doc.bind(on_release=lambda _,x=x:self.codeline(x=2))  
        self.code_run.bind(on_release=lambda _,x=x:self.codeline(x=3))
        
        ctnt.add_widget(self.codetext)
        btn_box.add_widget(self.code_del)
        btn_box.add_widget(self.code_doc)
        btn_box.add_widget(Widget())
        btn_box.add_widget(self.code_run)
        ctnt.add_widget(btn_box)
        
        popint_x=Window.size[0]/1.1
        popint_y=Window.size[1]/1.75
        self.popup = Popup(title='Python CodeLine [Live]', title_color='black',content=ctnt,size_hint=(None,None),size=(popint_x,popint_y),separator_color='grey',
                      auto_dismiss=True, background_color = (255,255,255,1),pos_hint={'top':0.86}) 
    
        self.popup.open()

    def codeline(self,x): #PythonLiveShell
        import sys
        FORBIDDEN_IMPORTS = [
    "os", "sys", "subprocess", "shutil", "socket", 
    "importlib", "ctypes", "pty", "urllib", "requests"]
        if x==1:
            self.codetext.text=''
            codeline_data.clear()
            codeline_data.append('')
        elif x==2:
            webbrowser.open('https://docs.sympy.org/latest/tutorials/intro-tutorial/index.html#intro-tutorial')
...
"""
            codedata= txt_add+self.codetext.text  
            codeline_data.append(self.codetext.text) #for save and back open after popup

            for imp in FORBIDDEN_IMPORTS:
                if f"import {imp}" in codedata or f"from {imp}" in codedata:
                    self.ids.lbl0.text='Python [color=#72ABFF]Live[/color]'
                    self.ids.lbl1.text = "unexpected error"
                    return
                

            if 'print(' in codedata:
                ...
                try:
                    ...
                    try:
                        ...
                        txt_end=output
                        sign0=sympify(txt_end)
                        sign_str=pretty(simplify(str(sign0)))    
                        sign_exp=sign_str.replace("ℯ","e") 
                        self.ids.lbl1.text=pretty(sign_exp) 
                    except(Exception,SympifyError) as e:
                        output = f"Error: {str(e)}"
                        self.ids.lbl1.text=str(e)

                self.ids.lbl0.text='Python [color=#72ABFF]Live[/color]'

            else:
                self.ids.lbl0.text='Python [color=#72ABFF]Live[/color]'
                self.ids.lbl1.text='[color=#AB80FF]print()[/color] needed to get output'
...
class HelpScreen(Screen):
    try:
        def help_gif_pre(self, topic):
            self.gif_t -= 1 #--------------timer for gif. 

            if topic == 3 or topic ==6:
                ...
                if self.gif_t== 0:
                    ...
                    self.img_help.source='algebra'+str(self.gif_t)+'.gif'
            elif topic ==2:
                if self.gif_t== 0:
                    self.b1.disabled=True
                else:
                    self.b2.disabled=False
                    self.img_help.source='calculus'+str(self.gif_t)+'.gif'
            elif topic ==4:
                if self.gif_t== 0:
                    self.b1.disabled=True
                else:
                    self.b2.disabled=False
                    self.img_help.source='matrices'+str(self.gif_t)+'.gif'
            elif topic ==5:
                if self.gif_t== 0:
                    self.b1.disabled=True
                else:
                    self.b2.disabled=False
                    self.img_help.source='graphs'+str(self.gif_t)+'.gif'
                ...
                if self.gif_t== 0:
                    ...
                    self.b2.disabled=False
                    self.img_help.source='vectors'+str(self.gif_t)+'.gif'
                ...
                if self.gif_t== 0:
                    ...
                    self.b2.disabled=False
                    self.img_help.source='special'+str(self.gif_t)+'.gif'

        def help_gif_next(self, topic): 
            self.gif_t += 1
        ...
        def help_topics(self, topic): 
            self.gif_t = 1

            ctnt=BoxLayout(orientation='vertical',spacing='1dp') 
            self.img_help=Image(source='', allow_stretch=True, keep_ratio=False, anim_delay=0.65) #https://ezgif.com/maker  -->en iyisi
            box_btn = BoxLayout(orientation='horizontal',spacing='5dp',size_hint_y=0.1)
            self.b1=Button(text='Previous',background_normal='#F0F0F0',background_color='#F0F0F0',color='black')
            self.b2=Button(text='Next',background_normal='#F0F0F0',background_color='#F0F0F0',color='black')
            box_btn.add_widget(self.b1)
            box_btn.add_widget(self.b2)
            ctnt.add_widget(self.img_help)
            ctnt.add_widget(box_btn)

            self.b1.bind(on_release=lambda _,topic=topic:self.help_gif_pre(topic))
            self.b2.bind(on_release=lambda _,topic=topic:self.help_gif_next(topic)) #belki bunları lambda ile yap. n=1, n=2 gibi 

            if topic == 1:
                self.img_help.source='algebra1.gif'
                ttopic = 'Algebra'
            elif topic == 2:
                self.img_help.source='calculus1.gif'
                ttopic = 'Calculus'
            elif topic == 3:
                self.img_help.source='diffeq.gif'
                ttopic = 'Differentiation Equations'
            elif topic == 4:
                self.img_help.source='matrices1.gif'
                ttopic = 'Matrices'
            elif topic == 5:
                self.img_help.source='graphs1.gif'
                ttopic = 'Graphs'
            elif topic == 6:
                self.img_help.source='series.gif'
                ttopic = 'Series'
            elif topic == 7:
                self.img_help.source='vectors1.gif'
                ttopic = 'Vector Fields'
            elif topic == 8:
                self.img_help.source='special1.gif'
                ttopic = 'Special Functions'
     
            popmat_x=Window.size[0]/1.1 
            popmat_y=Window.size[1]/2
            popup = Popup(title=ttopic,title_color='black',content=ctnt,size_hint=(None,None),size=(popmat_x,popmat_y),auto_dismiss=True,
                              background_color = (255,255,255,1),separator_color='grey')
            popup.open()
    except(Exception):
        pass
...
Then a few group members called [color=407EA8]Stars of the Sky[/color]. Our mission is, developing scientific useful-tools for all students or education field.[/i]
...
Also there is [b]Step-by-step[/b] solution for calculus. This solution need internet connection and linked you new tab on Google to open Sympy Gamma. \
This solution source is Sympy Gamma.
"""                    
    def write_last(self): 
        self.ids.helplabel_last.text="""
You can now download MathPath Console for Windows, and the whole project is [b]fully open source on GitHub![/b] Check out our web-site below. If you have feedback or you found a bug please contact us. \


• Use [b]Sympy Gamma[/b] (free) for [b]step-by-step[/b] solution on Web
[color=#777777]Update tag: [/color][color=#D26200]#Panda[/color]
"""
...
class MathPathApp(MDApp):
    ...
    def on_start(self):
        ...
        self.root.get_screen('menu').ids.pycode.icon="file-code-outline"        
